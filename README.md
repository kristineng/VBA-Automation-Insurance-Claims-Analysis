...
The project was designed to demonstrate how Excel and VBA can be applied to an insurance/risk analytics context rather than using Excel only for basic spreadsheet calculations.
...
- Clean and standardize raw insurance claims data
- Identify missing, invalid, and duplicate records
- Create derived insurance-related variables
- Analyze claim frequency and claim amounts
- Examine claims across different claim characteristics
- Analyze fraudulent versus non-fraudulent claims
- Examine customer and policy-level claim behavior
- Analyze claims over time
- Automate repetitive processing using VBA
- Build an Excel dashboard to communicate the results

---

# 📊 Dataset

The dataset contains claim-level insurance observations.
...
Additional variables were derived during the cleaning and analysis process.
...
The original dataset is kept separately in the `Raw_Data` worksheet.

The raw dataset is not directly modified during the analysis process.

Instead, a separate `Clean_Data` worksheet is created for processing.

This allows the original data to be preserved and provides a clear distinction between raw and processed data.
...
For example, claim amounts originally stored with currency symbols such as:

`$2,980`

are converted into numeric values so that they can be used correctly in calculations and analysis.

The formatted value is then displayed as:
...
Rather than automatically deleting problematic records, the project records data-quality issues and preserves the observations for further review.

A `data_quality_flag` variable was created to identify records requiring attention.
...
Claims are analyzed according to the police report indicator.

Both the number of claims and associated claim amounts are compared.
